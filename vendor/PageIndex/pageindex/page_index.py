...
async def check_title_appearance(item, page_list, start_index=1, model=None):    
    ...
    response = await gemini_generate_async(model=model, prompt=prompt)
...
def toc_detector_single_page(content, model=None):
    ...
    response = gemini_generate(model=model, prompt=prompt)
...
def extract_toc_content(content, model=None):
    ...
    response, finish_reason = gemini_generate_with_finish_reason(model=model, prompt=prompt)
    ...
    new_response, finish_reason = gemini_generate_with_finish_reason(model=model, prompt=prompt, chat_history=chat_history)
...
def toc_transformer(toc_content, model=None):
    ...
    last_complete, finish_reason = gemini_generate_with_finish_reason(model=model, prompt=prompt)
    ...
    while not (if_complete == "yes" and finish_reason == "finished"):
        ...
        new_complete, finish_reason = gemini_generate_with_finish_reason(model=model, prompt=prompt)
...
def add_page_number_to_toc(part, structure, model=None):
    ...
    current_json_raw = gemini_generate(model=model, prompt=prompt)
...
def generate_toc_continue(toc_content, part, model="models/gemini-2.5-flash"):
...
################### fix incorrect toc #########################################################
def single_toc_item_index_fixer(section_title, content, model="models/gemini-2.5-flash"):
    toc_extractor_prompt = """
